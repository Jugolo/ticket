...
set_error_handler(function($errno, $errstr, $errfile, $errline){
  ...
  $db->query("INSERT INTO `error` (
    ...
    '".$db->real_escape_string($errno)."',
    '".$db->real_escape_string($errstr)."',
    '".$db->real_escape_string($errfile)."',
    '".$db->real_escape_string($errline)."',
    NOW()
  );");
...
register_shutdown_function(function(){
  ...
  if($error){
    $db->query("INSERT INTO `error` (
      ...
      '".$db->real_escape_string($error["type"])."',
      '".$db->real_escape_string($error["message"])."',
      '".$db->real_escape_string($error["file"])."',
      '".$db->real_escape_string($error["line"])."',
      NOW()
    )");
  }
  if(defined("DB_INIT")){
...
function get_age(array $data){
  if(!empty($_POST["bd"]) && !empty($_POST["bm"]) && !empty($_POST["by"])){
    ...
    $db->query("UPDATE `user` SET 
                `birth_day`='".$db->real_escape_string(intval($_POST["bd"]))."',
                `birth_month`='".$db->real_escape_string(intval($_POST["bm"]))."',
                `birth_year`='".$db->real_escape_string(intval($_POST["by"]))."'
               WHERE `id`=".user["id"]);
...
function get_user_age(){
   $year = date("Y") - user["birth_year"];
   if((date("d") - user["birth_day"]) < 0 || (date("m") - user["birth_month"]) < 0)
     $year--;
   return $year;
}
...
function show_users(){
  ...
  while($row = $query->fetch_assoc()){
    echo two_container(htmlentities($row["username"]), ($group["changeGroup"] == 1 ? "<a href='?view=changegroup&uid=".$row["id"]."'>Change group</a>" : "").($row["id"] == user["id"] ? "" : " <a href='?view=users&delete={$row["id"]}'>Delete</a>"));
  }
}
...
function show_handleGroup(){
  ...
  if(!empty($_GET["standart"])){
    $db->query("UPDATE `group` SET `isStandart`=1 WHERE `id`='".(int)$_GET["standart"]."'");
    if($db->affected_rows == 0){
      html_error("Unknown group");
    }else{
      $db->query("UPDATE `group` SET `isStandart`=0 WHERE `id` <> '".(int)$_GET["standart"]."' AND `isStandart`=1");
  ...
  $query = $db->query("SELECT `id`, `name`, `isStandart` FROM `group`");
  while($row = $query->fetch_assoc()){
    echo two_container($row["name"], "<a href='?view=handleGroup&gid=".$row["id"]."'>Delete group</a> 
                                      <a href='?view=access&gid=".$row["id"]."'>Change access</a>".(
                                      $row["isStandart"] == 1 ? "" : " <a href='?view=handleGroup&standart={$row["id"]}'>Set standart</a>"
                      ));
  }
...
function saveTicketsSetting(array $data){
  $sql = [];
  
  if(!empty($_POST["age"]) && $_POST["age"] != $data["age"]){
    $sql["age"] = intval($_POST["age"]);
  }elseif(empty($_POST["age"]) && $data["age"]){
    $sql["age"] = "";
  }
  
  if(count($sql) != 0){
    $s = "UPDATE `catogory` SET";
    foreach($sql as $key => $value){
      $s .= " `".$key."`='".$value."'";
    }
    database()->query($s." WHERE `id`='".$data["id"]."'");
    html_okay("The setting is now updated");
  }else{
    html_error("No updates detected");
  }
  header("location: #");
  exit;
}
...
function adminTicket(){
  $db = database();
  $data = $db->query("SELECT * FROM `catogory` WHERE `id`='".$db->real_escape_string($_GET["tid"])."'")->fetch_assoc();
  if(!$data){
  ...
  echo "<form method='POST' action='#'>";
  echo two_container("Age in year", "<input type='number' name='age' value='".$data["age"]."'>");
  echo "<input type='submit' value='Save' name='setting'>";
  echo "</form>";
}
...
function show_error(){
  ...
  while($row = $query->fetch_assoc()){
    ...
    $table->td($row["errno"]);
    $table->td($row["errstr"]);
    $table->td($row["errfile"]);
    $table->td($row["errline"]);
    $table->td($row["errtime"]);
  }
...
function getMenu(){
  echo "<div id='menu'>";
    ...
    if(defined("user")){
      ...
      echo "<td><a href='?view=tickets' id='ticketmenu'>Show tickets</a></td>";
      if(showUserMenu()){
      ...
      if($group["showError"] == "1"){
        echo "<td><a href='?view=error'>Error</a></td>";
      }
    }
      echo "</tr>";
    echo "</table>";
